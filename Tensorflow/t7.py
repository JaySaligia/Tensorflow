...
def lda(x_raw, x_raw_1, x_raw_2, x_raw_3 ,d):#x_raw为输入矩阵（n*p），d为维数
    with tf.name_scope("lda"):
        ...
        for i in range(200):
            mean_diff = tf.concat([mean_diff, mean_2], 0)
        for i in range(200):
            mean_diff = tf.concat([mean_diff, mean_3], 0)
        #计算类内散度矩阵Sw
